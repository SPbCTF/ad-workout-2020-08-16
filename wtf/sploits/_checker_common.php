...
function randsentence($count = false, $charset = "LO") {
    $badwords = ["/storage/", "rm -rf", "cat", "ls", "dir"];
    ...
    for ($i = 0; $i < $count; $i++) {
        if (mt_rand(0, 20) == 0) {
            $word = $badwords[mt_rand(0, count($badwords) - 1)];
        } else {
            $word = randstr(mt_rand(3, 8), $charset);
        }
        $str .= $word . " ";
        if ($i != $count - 1 && mt_rand(0, 5) == 0) {
            $str = substr($str, 0, -1) . ", ";
        }
    }
    $str = substr($str, 0, -1);
